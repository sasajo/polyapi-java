...
abstract class AbstractClassGenerator<T extends Specification> {
  ...
  private File getClassDirectory(String packagePath) {
  ...
  private JsonNode normalizeObjectSchema(JsonNode rootSchema, JsonNode schema) {
    ...
    } else if (AbstractClassGenerator.isArray(schema)) {
  ...
  private String getWrappedType(ObjectPropertyType propertyType, String typeName) {
    if (propertyType.getSchema() != null) {
      if (AbstractClassGenerator.isArray(propertyType.getSchema())) {
